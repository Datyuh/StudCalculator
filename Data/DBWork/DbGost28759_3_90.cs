...
namespace StudCalculator.Data.DBWork
{
    class DbGost28759_3_90
    {
        ...
        {
            var executedBFromGost28759390 = Convert.ToDouble(db.GOST_28759_3_90.Where(p => p.PN == pn && p.DN == dn).Select(p => p.b).First());
            return executedBFromGost28759390;
        }

        public string ExecutionThicknessFlangeTheard(string pn, string dn)
        {
            var executionThicknessFlangeTheard = db.GOST_28759_3_90.Where(p => p.PN == pn && p.DN == dn).Select(p => p.Thread).First();
            return executionThicknessFlangeTheard;
        }

        public double ExecutionThicknessFlangeN(string pn, string dn)
        {
            var executionThicknessFlangenType1 = Convert.ToDouble(db.GOST_28759_3_90.Where(p => p.PN == pn && p.DN == dn).Select(p => p.n).First());
