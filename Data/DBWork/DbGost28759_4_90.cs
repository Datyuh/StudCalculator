...
namespace StudCalculator.Data.DBWork
{
    class DbGost28759_4_90
    {
        ...
        {
            var executePnCollection = new ObservableCollection<string>(db.OGK_StudCalculator_GOST_28759_4_90.Select(p => p.PN).Distinct().AsParallel());
        ...
        {
            var executeExecutionsCollection = new ObservableCollection<string>(db.OGK_StudCalculator_GOSTs.Select(p => p.Exec_GOST28759_4).Where(p => p != null)).AsParallel();
        ...
        {
            var executeDnCollection = new ObservableCollection<string>(db.OGK_StudCalculator_GOST_28759_4_90.Select(p => p.DN)).Distinct().AsParallel();
        ...
        {
            var executedBFromGost28759390 = Convert.ToDouble(db.OGK_StudCalculator_GOST_28759_4_90.Where(p => p.PN == pn && p.DN == dn).Select(p => p.b).First());
            return executedBFromGost28759390;
        }

        public string ExecutionThicknessFlangeTheard(string pn, string dn)
        {
            var executionThicknessFlangeTheard = db.OGK_StudCalculator_GOST_28759_4_90.Where(p => p.PN == pn && p.DN == dn).Select(p => p.Thread).First();
            return executionThicknessFlangeTheard;
        }

        public double ExecutionThicknessFlangeN(string pn, string dn)
        {
            var executionThicknessFlangenType1 = Convert.ToDouble(db.OGK_StudCalculator_GOST_28759_4_90.Where(p => p.PN == pn && p.DN == dn).Select(p => p.n).First());
