...
namespace StudCalculator.Data.DBWork
{
    internal class DbWorkGost33259 : BaseViewModel
    {
        ...
        {
            var pn = new ObservableCollection<string>(db.OGK_StudCalculator_Execution_All.Select(p => p.PN).Distinct());
        ...
        {
            var dn = new ObservableCollection<string>(db.OGK_StudCalculator_Execution_All.Select(p => p.DN).Distinct());
        ...
        {
            var typeGost33259 = new ObservableCollection<string>(db.OGK_StudCalculator_GOSTs.Select(p => p.Style).Where(p => p == "Тип 11"));
            return typeGost33259;
        }

        public string ExecutionThicknessFlangeTheard1(string pn, string dn)
        {
            var executionThicknessFlangeTheard1 = db.OGK_StudCalculator_Type_11.Where(p => p.PN == pn && p.DN == dn).Select(p => p.Thread_1).First().ToString();
            return executionThicknessFlangeTheard1;
        }

        public double ExecutionThicknessFlangeb1(string pn, string dn)
        {
            var executionThicknessFlangeb1 = Convert.ToDouble(db.OGK_StudCalculator_Type_11.Where(p => p.PN == pn && p.DN == dn).Select(p => p.b_1).First());
            return executionThicknessFlangeb1;
        }

        public double ExecutionThicknessFlangen_type1(string pn, string dn)
        {
            var executionThicknessFlangenType1 = Convert.ToDouble(db.OGK_StudCalculator_Type_11.Where(p => p.PN == pn && p.DN == dn).Select(p => p.n_type1).First());
